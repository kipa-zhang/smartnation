...
//			console.info(args);
			...
			height = item.height(),
			offset = {top: 0, left: 0};
//			console.info(item.context.parentNode.style.width);
			if (ratio.h > ratio.w) {
...
//						.width(width).attr('width', width + 'px');
			return item.height(height).attr('height', '100%')
						.width(width).attr('width', '100%');
		},
			...
			_init = function () {
				var item = $(this),
					...
					args = { item : item,
							settings : settings,
							wrapper : wrapper };
					settings.wrapperWidth = wrapper.css('width');
					settings.wrapperHeight = wrapper.css('height');
//					console.info(wrapper.css('width'));
//					console.info(wrapper.css('height'));
